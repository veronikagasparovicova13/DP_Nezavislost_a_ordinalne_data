...
ppA <- as.vector(outer(row_sums, col_sums)) # nerovnomerna pp. (platnost H0)
...
# gamma3x4 = 0.09691314 -> gamma2x3 = 0.4153132
...
for (s in 1:n_sim) {
  ...
  tbl1 <- matrix(hadik, r, c)
  tbl2 <- collapse_3x4_to_2x3_r12_c12(tbl1)
  
  # Vypocet gamma a sigma_gamma
  sigma_3x4 <- gamma_1hadik(tbl1)
  sigma_2x3 <- gamma_1hadik(tbl2)
  ...
  gamma_3x4 <- (C1 - D1) / (C1 + D1)
  gamma_2x3 <- (C2 - D2) / (C2 + D2)
  
  # Z-test pre gamma = 0
  z_3x4[s] <- gamma_3x4 / sigma_3x4
  p_value_3x4 <- 2 * (1 - pnorm(abs(z_3x4[s])))
  rej_3x4[s] <- p_value_3x4 < 0.05
  
  z_2x3[s] <- gamma_2x3 / sigma_2x3
  p_value_2x3 <- 2 * (1 - pnorm(abs(z_2x3[s])))
  rej_2x3[s] <- p_value_2x3 < 0.05
}

# Vykreslenie
k_3x4 <- sum(rej_3x4, na.rm = TRUE) # pocet zamietnuti pre tabulku: 1 had
n_3x4 <- sum(!is.na(rej_3x4)) # pocet platnych zamietnuti pre tabulku: 1 had (bez NA)
...
ggplot(df_alpha, aes(x = scenár, y = alpha)) +
  ...
  labs(title = "Pravdepodobnosť chyby I. druhu pre obe tabuľky pre Gamma",
       x = NULL, y = "Odhad alpha") +
